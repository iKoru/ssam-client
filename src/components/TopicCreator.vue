    ...
    <v-card-text>
      <v-form>
        <v-layout row xs12 wrap>
          ...
          <v-flex xs12>
            <v-autocomplete name="allowedGroups" chips multiple item-text="text" item-value="value" v-model="allowedGroups" :disabled="allGroupAuth === 'READWRITE'" :items="groupItems" label="구독허용 그룹" hint="(최소) 내가 구독할 수 있도록 선택해야 합니다." persistent-hint>
              <template slot="selection" slot-scope="props">
                <v-chip close :key="props.item.value" :selected="props.selected" @input="removeChip(props, props.item, allowedGroups)">{{props.item.text}}</v-chip>
              </template>
            </v-autocomplete>
          </v-flex>
        </v-layout>
...
<script>
const reserved = ['document', 'documents', 'profile',
...
export default {
  ...
  created(){
    this.$axios
      ...
      .then(response => {
        this.groupItems = response.data.map(x => ({text: x.groupName, value: x.groupId}));
      })
      .catch(error => {
